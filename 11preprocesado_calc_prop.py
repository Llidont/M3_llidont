...
calc_combined = calc_combined.loc[
    (calc_combined['ROI_mask_file_path'].str.startswith('datasets/archive/')) &
    (calc_combined['image_file_path'].str.startswith('datasets/archive/'))]
...
if(guardarimagenes):
    ...
    display_images(calc_combined, 'image_file_path', 5, 'examples/calc_prop_image.jpeg')
    display_images(calc_combined, 'ROI_mask_file_path', 5, 'examples/calc_prop_roi.jpeg')
...
if(imagenesprocesadas): 
    process_and_display_images(calc_combined, 5, required_dim, 'calc_prop', all_prop=True)


procesar_imagenes=True
if(procesar_imagenes):
    os.makedirs("datasets/calc_prop/ROI", exist_ok=True)
    os.makedirs("datasets/calc_prop/image", exist_ok=True)
    labels = []
    for i, row in calc_combined.iterrows():
        ...
        final_mask.save(f"datasets/calc_prop/ROI/ROI" + image_index + ".png")
        final_image.save(f"datasets/calc_prop/image/image" + image_index + ".png")
        # Añadimos al diccionario
        labels.append({"index": image_index, "label": row["pathology"],
                       "ROI_path": "datasets/calc_prop/ROI/" + "ROI" + image_index + ".png",
                       "image_path": "datasets/calc_prop/image/" + "image" + image_index + ".png",
    ...
    labels_df.to_csv("datasets/calc_prop/calc_prop_info.csv", index=False)
