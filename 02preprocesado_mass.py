...
if(procesar_imagenes):
    os.makedirs("datasets/mass_clean/ROI", exist_ok=True)
    os.makedirs("datasets/mass_clean/image", exist_ok=True)
    labels = []
    for i, row in mass_combined.iterrows():
        ...
        labels.append({"index": image_index, "label": row["pathology"],
                       "ROI_path": "datasets/mass_clean/ROI/" + "ROI" + image_index + ".png",
                       "image_path": "datasets/mass_clean/image/" + "image" + image_index + ".png",
    ...
    labels_df.to_csv("datasets/mass_clean/mass_clean_info.csv", index=False)
